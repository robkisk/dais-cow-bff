...
from pyspark.sql.functions import col, countDistinct, sum, udf
from pyspark.sql.types import IntegerType

calculate_time_overlap = (
    lambda start_interval_1, end_interval_1, start_interval_2, end_interval_2: 0
    if end_interval_1 <= start_interval_2 or end_interval_2 <= start_interval_1
    else min(end_interval_1, end_interval_2) - max(start_interval_1, start_interval_2)
)
...
def compute_heatmap(cows_bff: DataFrame):
    cow1 = (
        cows_bff.withColumnRenamed("cow_name", "cow1")
    ...
    cow2 = (
        cows_bff.withColumnRenamed("cow_name", "cow2")
    ...
    df = cow1.crossJoin(cow2).where(
        (cow1.cow1 != cow2.cow2) & (cow1.date1 == cow2.date2)
    )

    df = df.withColumn(
        "overlap",
        calculate_time_overlap_udf(
            "meal_start1", "meal_end1", "meal_start2", "meal_end2"
        ),
    ).select("cow1", "cow2", "date1", "overlap")

    df = df.groupBy("cow1", "cow2").agg(
        sum("overlap").alias("total_overlap"),
        countDistinct("date1").alias("distinct_days"),
    )
    df = (
        df.withColumn("avg_overlap", df.total_overlap / df.distinct_days)
